...
-- Quota years are stored in a deterministic ascending order. Keeping this
-- validation in the database protects historical imports as well as the RPC.
...
begin
  ...
  -- Keep accepting quota_year for older clients. New clients send quota_years;
  -- quota_year remains the first canonical (ascending) year on every receipt.
